...
| Daily-use MCP surface | Full 14-tool surface, with `v0.6.0` widening `search_graph` via BM25 `query` search and vector-backed `semantic_query`; `ingest_traces` remains stubbed | All 13 previously meaningful shared tools are implemented, Zig now also ships the released BM25 `search_graph.query` path and the released `index_repository(mode=\"moderate\")` contract, but not the vector-backed `semantic_query` mode | `Partial` | No |
| Core indexing pipeline | Broad multi-pass pipeline including `full` / `moderate` / `fast` modes, `SIMILAR_TO`, `SEMANTICALLY_RELATED`, persisted `IMPORTS`, route/data-flow, channel edges, tests, config links, infra scans, and git history | Strong core pipeline for structure, definitions, calls, usages, incremental, parallel, explicit `fast` / `moderate` / `full` pass selection, similarity, route/event slices, plus embedded FTS5 refresh and optional SCIP sidecar import | `Partial` | No |
...
As of `2026-04-21`, the pre-`v0.6.0` shared contract is still largely closed. The newest latest-upstream gaps are now concentrated in `search_graph.semantic_query`, `SEMANTICALLY_RELATED`, and the newer channel graph vocabulary (`Channel`, `EMITS`, `LISTENS_ON`). Older intentional scope choices such as the UI binary, infra/resource indexing, and deeper Cypher/LSP breadth still remain.
...
| `tools/list` | Yes, advertises 14 tools | Yes, but the latest-upstream advertised schema still differs in visible ways | `Partial` | No | The exact `tool-surface-parity` fixture now locks tool inventory, `repo_path`, the visible `ingest_traces` stub, the real Zig-side `moderate` mode contract, and the Zig-side `search_graph.query` surface, but full parity is still blocked by the remaining latest-upstream `search_graph.semantic_query` schema and payload. |
...
|------|------------|----------|--------|----------------|-------|
| `index_repository` | Full | Implemented with a real `full` / `moderate` / `fast` public contract and an explicit Zig pass split, but without the upstream semantic-search enrichment coupled to `moderate` / `full` in `v0.6.0` | `Partial` | No | The exact `tool-surface-parity` fixture now proves `repo_path` compatibility and accepted `moderate` indexing. This row stays below full parity because the latest-upstream `moderate` / `full` semantics still depend on `SEMANTICALLY_RELATED` and vector-backed semantic search that Zig does not ship yet. |
| `search_graph` | Full, with `name_pattern`, BM25 `query`, and vector-backed `semantic_query` discovery modes | Implemented with rich structured filters and pagination, plus the released BM25 `query` path over the local FTS substrate, but without upstream `semantic_query` | `Partial` | No | Zig now matches the structured filter surface plus the released lexical discovery path, including `search_mode:"bm25"` and ranked node rows, but it still lacks the vector-backed `semantic_query` path and `semantic_results` payload. |
...
| `search_graph` discovery modes | Regex-style `name_pattern`, BM25 `query`, and vector-backed `semantic_query` | Regex-style `name_pattern` plus BM25 `query`, but no `semantic_query` | The newest contract gap is now the vector-backed semantic mode rather than the lexical discovery path. |
...
| Tools advertised via `tools/list` | 14 total, including stub `ingest_traces` | Tool inventory is converging, but exact schema parity still depends on unresolved latest-upstream contract deltas | Exact tool-surface parity is no longer just a count question; the remaining visible gap is the latest-upstream `search_graph.semantic_query` contract and the related search-graph schema drift around it. |
...
| Index modes | `full`, `moderate`, `fast` | `full`, `moderate`, `fast` | `Partial` | No | Zig now exposes the released three-mode public contract and gives `fast`, `moderate`, and `full` explicit pass-selection behavior. This row remains below full parity because the latest-upstream `moderate` / `full` modes still carry semantic-search and `SEMANTICALLY_RELATED` behavior that Zig does not implement. |
