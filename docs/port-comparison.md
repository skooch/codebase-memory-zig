...
| Daily-use MCP surface | Full 14-tool surface, with `v0.6.0` widening `search_graph` via BM25 `query` search and vector-backed `semantic_query`; `ingest_traces` remains stubbed | All 13 previously meaningful shared tools are implemented, and Zig now also ships the released BM25 `search_graph.query`, vector-backed `search_graph.semantic_query`, and the released `index_repository(mode=\"moderate\")` contract | `Near parity` | Yes |
| Core indexing pipeline | Broad multi-pass pipeline including `full` / `moderate` / `fast` modes, `SIMILAR_TO`, `SEMANTICALLY_RELATED`, persisted `IMPORTS`, route/data-flow, channel edges, tests, config links, infra scans, and git history | Strong core pipeline for structure, definitions, calls, usages, incremental, parallel, explicit `fast` / `moderate` / `full` pass selection, similarity, persisted semantic vectors plus `SEMANTICALLY_RELATED`, route/event slices, plus embedded FTS5 refresh and optional SCIP sidecar import | `Partial` | No |
...
As of `2026-04-21`, the pre-`v0.6.0` shared contract is still largely closed. The remaining latest-upstream graph-model delta is now concentrated in the newer channel vocabulary (`Channel`, `EMITS`, `LISTENS_ON`) rather than the semantic-search layer. Older intentional scope choices such as the UI binary, infra/resource indexing, and deeper Cypher/LSP breadth still remain.
...
| `tools/list` | Yes, advertises 14 tools | Yes, including the released `search_graph.semantic_query` schema | `Near parity` | Yes | The exact `tool-surface-parity` fixture now locks tool inventory, `repo_path`, the visible `ingest_traces` stub, the real Zig-side `moderate` mode contract, and the released `search_graph.query` plus `semantic_query` schema. It stays diagnostic-only in the full compare because the local C checkout used there predates `v0.6.0`, not because Zig still lacks the surface. |
...
|------|------------|----------|--------|----------------|-------|
| `index_repository` | Full | Implemented with a real `full` / `moderate` / `fast` public contract, an explicit Zig pass split, persisted semantic vectors, and bounded `SEMANTICALLY_RELATED` emission in `moderate` / `full` | `Near parity` | Yes | The exact `tool-surface-parity` fixture now proves `repo_path` compatibility and accepted `moderate` indexing, and the semantic contract fixture locks the released semantic enrichment behavior on the Zig side. |
| `search_graph` | Full, with `name_pattern`, BM25 `query`, and vector-backed `semantic_query` discovery modes | Implemented with rich structured filters and pagination, plus the released BM25 `query` path and vector-backed `semantic_query` with separate `semantic_results` rows | `Near parity` | Yes | Zig now matches the structured filter surface, the released lexical discovery path, and the released semantic discovery path, including `search_mode:"bm25"` and `semantic_results`. |
...
| `search_graph` discovery modes | Regex-style `name_pattern`, BM25 `query`, and vector-backed `semantic_query` | Regex-style `name_pattern`, BM25 `query`, and vector-backed `semantic_query` | The discovery-mode gap is closed; the remaining latest-upstream graph-model delta lives below the search surface in channel vocabulary and message-edge naming. |
...
| Tools advertised via `tools/list` | 14 total, including stub `ingest_traces` | 14 total, including the released `search_graph.semantic_query` schema and stub `ingest_traces` | Exact tool-surface parity is now locked on the Zig side; the full compare keeps this row diagnostic-only because the local C comparator is older than the `v0.6.0` release baseline. |
...
| Index modes | `full`, `moderate`, `fast` | `full`, `moderate`, `fast` | `Near parity` | Yes | Zig now exposes the released three-mode public contract, gives `fast`, `moderate`, and `full` explicit pass-selection behavior, and refreshes semantic vectors and `SEMANTICALLY_RELATED` edges in `moderate` / `full`, including the incremental no-change path. |
...
| Semantic relatedness / embedding layer | `SEMANTICALLY_RELATED` edges plus vector data that powers `search_graph.semantic_query` in `moderate` / `full` modes | Implemented with a local lightweight vector substrate, persisted semantic vectors, bounded `SEMANTICALLY_RELATED` emission, and `semantic_query` search | `Near parity` | Yes | The new `semantic-query-contract` fixture now locks the Zig-side semantic query payload and exact `SEMANTICALLY_RELATED` row shape on the seeded send-task fixture. |
...
| `SEMANTICALLY_RELATED` | Yes | Yes | `Near parity` | Yes | Zig now emits bounded semantic-similarity edges with `score` and `same_file` properties and uses the same substrate to power `search_graph.semantic_query`. |
