...
| Route-node creation / cross-service graph | Yes | Partial | `Partial` | No | Zig now classifies call edges as `HTTP_CALLS`/`ASYNC_CALLS`, creates stub and concrete URL/path `Route` nodes, emits verified decorator-backed `HANDLES`, and has a Zig-verified `DATA_FLOWS` bridge. A strict shared C/Zig `DATA_FLOWS` fixture remains open because the current C reference binary does not emit that row on the public route fixture. |
...
| Route graph (`Route`, `HTTP_CALLS`, `ASYNC_CALLS`, `HANDLES`, route-linked data flows) | Yes | Partial | `Partial` | No | Zig now creates `HTTP_CALLS` and `ASYNC_CALLS` edges, concrete URL/path `Route` nodes, verified decorator-backed `HANDLES`, and Zig-verified route-linked `DATA_FLOWS`. Broader framework coverage and a strict shared C/Zig `DATA_FLOWS` fixture remain open. |
...
| Remaining long-tail edge families | Yes (broader vocabulary) | Limited to verified slices | `Partial` | No | The remaining or unproven original-overlap edges are `OVERRIDE` (Go-only) and `WRITES`/`READS` (not proven by the current C reference fixture). `HANDLES` now has a verified decorator-backed route slice, and `DATA_FLOWS` has Zig coverage but still needs a strict shared C/Zig fixture. `CONTAINS_PACKAGE` was never implemented in C. |
...
| Partial route / cross-service graph stack | Zig now emits verified decorator-backed `HANDLES` and route-linked `DATA_FLOWS`, but broader framework coverage and strict shared `DATA_FLOWS` parity remain open. |
| No LSP-assisted hybrid resolution | Some higher-fidelity call/type resolution paths remain original-only. |
| No strict shared DATA_FLOWS fixture or full config normalization | Git-history coupling is implemented, config linking has dependency-import matching, decorator-backed `HANDLES` is verified, and Zig has route-linked `DATA_FLOWS`. The remaining graph-model gap is proving `DATA_FLOWS` against a shared C/Zig public fixture, broader framework route coverage, and the full config normalization surface. |
