...
- `bash scripts/run_interop_alignment.sh`: completed against the local C reference checkout with `1` bounded mismatch remaining in the broader parity surface (`go-parity/query_graph`)
...
| Readiness gate | Reference side of the interop harness | Completed and passing: `Strict matches: 58`, `Diagnostic-only comparisons: 9`, `Mismatches: 0` | `Near parity` | Yes | The first-gate harness is green; the expanded full harness now reports 30 fixtures, 230 comparisons, 132 strict matches, 34 diagnostic-only comparisons, 1 remaining mismatch, and `cli_progress: match`. |
